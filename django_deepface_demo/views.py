...
def device_stats_view(request):
    ...
    browser_stats = CaptureDevice.objects.values('browser').annotate(
        count=Count('id')
    ).order_by('-count')

    # Get OS statistics
    os_stats = CaptureDevice.objects.values('os').annotate(
        count=Count('id')
    ).order_by('-count')

    # Get device type statistics
    device_stats = CaptureDevice.objects.values('device_type').annotate(
        count=Count('id')
    ).order_by('-count')
